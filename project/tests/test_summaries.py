...
def test_remove_summary_incorrect_id(test_app_with_db):
    """
    GIVEN test_app_with_db
    WHEN the DELETE method is called with an invalid id
...
def test_update_summary_incorrect_id(test_app_with_db):
    """
    GIVEN test_app_with_db
    WHEN the PUT method is called with an invalid id
    THEN return a 404 response
    """
    response = test_app_with_db.put(
    ...
    response = test_app_with_db.put(
        "/summaries/0/", data=json.dumps({"url": "https://foo.bar", "summary": "updated!"})
    )
    assert response.status_code == 422
    assert response.json() == {
        "detail": [
...
def test_update_summary_invalid_json(test_app_with_db):
    """
    GIVEN test_app_with_db
    WHEN the PUT method is called with invalid json
    THEN return a 422 response
    """
    response = test_app_with_db.post("/summaries/", data=json.dumps({"url": "https://foo.bar"}))
    summary_id = response.json()["id"]
...
def test_update_summary_invalid_keys(test_app_with_db):
    """
    GIVEN test_app_with_db
    WHEN the PUT method is called with invalid keys
    THEN return a 422 response
    """
    response = test_app_with_db.post("/summaries/", data=json.dumps({"url": "https://foo.bar"}))
    summary_id = response.json()["id"]
